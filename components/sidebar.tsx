...
export function Sidebar({ isOpen, onToggle }: SidebarProps) {
  ...
  return (
    <>
      <div
        className={cn(
          "h-screen bg-background flex flex-col transition-all duration-300 ease-in-out z-40", // Common styles
          // Mobile (default): fixed, slides in/out
          "fixed top-0 left-0 w-64 border-r", // Base for mobile
          isOpen ? "translate-x-0" : "-translate-x-full", // Sliding behavior
          // Medium screens and up (md:): relative, in-flow
          "md:relative md:translate-x-0", // Revert to in-flow, reset transform
          isOpen ? "md:w-64" : "md:w-16" // Widths for md+
        )}
      >
