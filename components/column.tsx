...
export default function Column({
  ...
  return (
    <div className="shrink-0 w-72 flex flex-col bg-gray-800 rounded-md shadow-sm">
      <div
        className={`p-3 flex justify-between items-center border-b border-gray-700 rounded-t-md ${headerColorClass}`}
      >
        <h3 className="font-medium text-sm uppercase tracking-wider text-gray-300 flex items-center">
          {column.title}
          <span className="ml-2 text-xs bg-gray-700 text-gray-400 px-2 py-0.5 rounded-full">{column.tasks.length}</span>
        </h3>
        <div className="flex">
          <Popover>
            ...
            <PopoverContent className="w-64 bg-gray-700 border-gray-600">
              <div className="space-y-2">
                <h4 className="font-medium text-sm text-gray-200">Column Color</h4>
      ...
      <Droppable droppableId={column.id}>
        {(provided) => (
          <div ref={provided.innerRef} {...provided.droppableProps} className="flex-1 p-2 overflow-y-auto">
            ...
            {isAddingTask ? (
              <div className="mt-2 p-3 bg-gray-700 rounded-md shadow-sm border border-gray-600">
                <Label htmlFor="task-title" className="text-gray-200">
                  Task Title
                </Label>
                <Input
                  id="task-title"
                  ...
                  className="mb-2 bg-gray-600 border-transparent focus:border-indigo-500 text-gray-200"
                />
                <Label htmlFor="task-description" className="text-gray-200">
                ...
                <div className="flex gap-2">
                  <Button size="sm" className="bg-indigo-500 hover:bg-indigo-600" onClick={handleAddTask}>
                    Add
                  </Button>
                  <Button
                    size="sm"
              ...
              <Button
                variant="ghost"
                className="w-full mt-2 text-gray-400 hover:text-gray-200 hover:bg-gray-700/50 justify-start"
